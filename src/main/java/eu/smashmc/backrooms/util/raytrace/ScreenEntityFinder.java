...
public class ScreenEntityFinder {
    ...
    public List<Entity> findEntities(double range) {
        List<Entity> entities = new ArrayList<>();
        Location origin = player.getEyeLocation();
        Vector direction = player.getLocation().getDirection();
        List<RaytraceHitbox> entityHitboxes = RaytraceHitboxHelper.collectPossibleEntityHitboxes(origin, direction, range, 120);
        for(RaytraceHitbox entityBox : entityHitboxes){
            if(entityBox instanceof RaytraceEntityHitbox entityBoxCast){
                if(entityBoxCast.getEntity().equals(player)) {
                    continue;
                }
            }
     ...
     * double outerX = clamp(eyeX, min.getX(), max.getX());
     * double outerY = clamp(eyeY, min.getY(), max.getY());
     * double outerZ = clamp(eyeZ, min.getZ(), max.getZ());
     *
     * @param hitbox
     * @return
     */
    private boolean isOcluded(RaytraceHitbox hitbox, double range) {
        ...
        Vector directionToCenter = center.clone().subtract(eyeLocation.toVector());
        ...
        for(Location point : points) {
            Vector directionToPoint = point.toVector().subtract(eyeLocation.toVector());
            ...
            for(var hit : hits){
                var distanceSqrt = hit.getPoint().distanceSquared(eyeLocation);
            ...
            // As we shoot an ray into the exact position of the target hitbox, it should always be found in the raytrace
            if(closestHit == null){
            ...
            if(closestHit instanceof RaytraceEntityHit){
                ...
                var hit = result.getHits().iterator().next();
            }
        }
        return true;
    }
